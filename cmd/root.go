...
func Parse(args []string, version string) (*config.Config, []string, error) {
	...
	fs.StringVar(&cfg.RotorBenchBin, "rotor-bench", cfg.RotorBenchBin, "[EXPERIMENTAL - currently broken, do not use] rotor-bench binary path for planar/iso KV types (optional)")
	...
	fs.StringVar(&cfg.DirCTV, "ctv-dir", cfg.DirCTV, "V-cache type sweep direction (up|down)")
	...
	fs.StringVar(&cfg.GoalSort, "goal-sort", cfg.GoalSort, "Sort Goal Results table by: tg (default), ctx, ngl, pp")
