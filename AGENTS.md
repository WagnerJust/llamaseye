...
**llamaseye** is a Go binary that exhaustively benchmarks every meaningful llama-bench parameter combination for GGUF models.
...
| 2 | FA + KV quant | Flash attention x KV cache type (f16, q8_0, q4_0, turbo2-4) |
...
Phase 6 fallback order on OOM: (1) flip `nkvo`, (2) try progressively more-compressed `ctk` types x both `nkvo` values. Only ctk/nkvo values already validated by Phases 2 and 4 are tried as fallbacks.
...
- **Git Safety:** NEVER use `git add .` or `git add -A`. Only stage specific files you modified.
- **No direct commits to main.** All work uses issue -> branch -> PR workflow.
- **Type safety:** Run `golangci-lint run` before committing. Fix all findings.
- **Test before commit:** Run `go test ./...` to verify all tests pass.
- **Plans and design notes are not tracked.** Implementation plans, design proposals, architecture sketches, and scratch notes go under `.local/plans/` or `.local/specs/` (gitignored). Do not commit them to `docs/`. The committed engineering doc is `docs/spec.md` only — that describes what the code *does*, not what an agent intends to do next.
